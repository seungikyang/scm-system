...
> **번호순으로 풀지 않습니다.** 번호는 기존 문제와 starter 경로를 유지하기 위한
> 식별자입니다. 처음 학습할 때는
> `00 → 01 → 02 → 03 → 04 → 07 → 08 → 09 → 10 → 11 → 15 → 16 → 17 → 18 → 19 → 21 → 29`
> 순으로 핵심 발주 흐름을 먼저 완성합니다. 그다음
> `20 → 23 → 24 → 25 → 28 → 31 → 32 → 33 → 34 → 35`,
> `22 → 36 → 37 → 38 → 39`를 진행하고, 제품 코드가 없는 확장 모듈
> `05 → 06 → 12 → 13 → 14 → 26 → 27 → 30`은 마지막에 풉니다.
> 각 단계의 목적은 [워크북 안내](./README.md#학습-트랙)를 참고하세요.
